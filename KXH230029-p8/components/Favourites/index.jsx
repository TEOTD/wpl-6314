...
import "./styles.css";
import {Paper, Typography, Button, Tooltip, IconButton, Dialog, DialogActions, DialogContent, DialogTitle} from "@mui/material";
...
function UserFavorites() {
        ...
        if(imageDetails){
            (async () => {
                await axios.get(`/user/${imageDetails.user_id}`)
                    .then((result) => {setImageUser(result.data);})
                    .catch((error) => console.error("Failed to fetch user photos:", error));
            })();
        }
    }, [imageDetails, reload]);
    ...
    {photoList.length>0? photoList.map((photo, index) => (
        ...
        >
          {/* Display the photo */}
          <Button sx={{justifyContent: "center"}} onClick={() => {setImageDetails(photo); setOpen(true);}}>
          <img
          ...
          />
            ...
            <Tooltip title="Remove Photo" arrow>
                <IconButton onClick={() => {RemoveFavourite(photo._id);}}>
                    <CancelIcon/>
                </IconButton>
            </Tooltip>
        </Paper>
    ...
    >
        ...
        <Dialog open={open} onClose={handleClose} sx={{
            ...
            <DialogContent>
                ...
                (
                <>
                    <img src={`/images/${imageDetails.file_name}`} alt={imageDetails.file_name} className="photo-image"/>
                    <Paper sx={{padding: "15px", margin:"4px"}} className="dialog-info-card">
                        <Typography variant="body1">{"Posted On: "+formatDateTime(imageDetails.date_time)}</Typography>
                    </Paper>
                    <Paper sx={{padding: "15px", margin:"4px"}} className="dialog-info-card">
                        <Typography variant="body1">{"Posted By: "+imageUser.first_name+" "+imageUser.last_name}</Typography>
                    </Paper>
                    <Paper sx={{padding: "15px", margin:"4px"}} className="dialog-info-card">
                        <Typography variant="body1">{"File Name: "+imageDetails.file_name}</Typography>
                    </Paper>
                    <Paper sx={{padding: "15px", margin:"4px"}} className="dialog-info-card">
                        <Typography variant="body1">{"Number of Comments: "+imageDetails.comments.length}</Typography>
                    </Paper>
